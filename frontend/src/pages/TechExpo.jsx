...
const EXPOS = Array.from({ length: 10 }, (_, i) => ({
  id: i + 1,
  title: `🏭 Tech Innovation Expo ${i + 1}: ${['Hardware', 'Software', 'GreenTech', 'AgriTech', 'SpaceTech', 'BioTech', 'AutoTech', 'Smart City', 'Wearables', 'AR/VR'][i]}`,
  desc: "Showcase your best Maker projects to industry leaders and investors. The perfect place to get your startup off the ground.",
  date: `November ${5 + i}, 2026`,
  location: `Main Auditorium, Hall ${i % 4 + 1}`,
  partners: `${(i + 2) * 10}+ Industry Partners`
}));

export const TechExpo = () => {
    ...
    <div className="bg-[var(--color-background)] text-[var(--color-text-primary)] font-sans min-h-screen pb-24 relative">
      <header className="bg-[var(--color-surface)] sticky top-0 z-50 flex justify-between items-center px-4 md:px-8 w-full h-16 border-b border-[var(--color-border)] shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl md:text-2xl font-black tracking-tight text-[var(--color-primary)]">Tech Expo</h1>
        </div>
      </header>
            ...
            <div key={event.id} className="card-standard p-8 flex flex-col">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-sm">{event.partners}</span>
                </div>
              </div>
              <button className="btn-primary w-full mt-auto">Book a Stall</button>
            </div>
          ))}
          </section>
