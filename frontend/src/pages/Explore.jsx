    ...
    <div className="pb-24">
      ...
      <div className="p-4 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-rows-3 grid-flow-col gap-4 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-4">
            {newsData.map(news => (
            ...
            {demoProblems.map(prob => (
              <div key={prob.id} className="snap-center shrink-0 w-[85vw] md:w-[300px] lg:w-[350px] bg-[var(--color-card)] rounded-3xl p-5 border border-[var(--color-border)] shadow-sm hover:border-[var(--color-primary)] transition-colors cursor-pointer group flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-base font-bold text-[var(--color-text-primary)] leading-tight mb-2 group-hover:text-[var(--color-primary)] transition-colors">
                    {prob.title}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)] mb-4 leading-relaxed line-clamp-3">
                    {prob.desc}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2 mt-auto">
                  {prob.tags.map(tag => (
                    <span key={tag} className="bg-[var(--color-background)] text-[var(--color-text-primary)] border border-[var(--color-border)] px-2 py-1 rounded-md text-[10px] font-bold shadow-sm">
                      {tag}
                    </span>
                  ))}
                </div>
