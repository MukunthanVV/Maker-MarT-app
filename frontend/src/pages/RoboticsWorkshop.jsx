...
const WORKSHOPS = Array.from({ length: 10 }, (_, i) => ({
  id: i + 1,
  title: `🤖 Robotics Workshop ${i + 1}: ${['Arduino', 'Raspberry Pi', 'Drone Building', 'Line Followers', 'MicroPython', 'ROS', '3D Printing', 'PCB Design', 'Motor Control', 'Sensors'][i]}`,
  desc: "Learn to build autonomous robots from scratch. No prior experience required! We provide all the microcontrollers and sensors.",
  date: `December ${12 + i}, 2026`,
  location: `Hardware Lab ${i % 5 + 1}`,
  perk: `Free Hardware Kit Level ${i % 3 + 1}`
}));
    ...
    <div className="bg-[var(--color-background)] text-[var(--color-text-primary)] font-sans min-h-screen pb-24 relative">
      <header className="bg-[var(--color-surface)] sticky top-0 z-50 flex justify-between items-center px-4 md:px-8 w-full h-16 border-b border-[var(--color-border)] shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl md:text-2xl font-black tracking-tight text-[var(--color-primary)]">Robotics Workshop</h1>
        </div>
      </header>
          ...
          {WORKSHOPS.map((event) => (
            <div key={event.id} className="card-standard p-8 flex flex-col">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-purple-500">precision_manufacturing</span>
                  <span className="font-semibold text-sm">{event.perk}</span>
                </div>
              </div>
              <button className="btn-primary w-full mt-auto">Join Workshop</button>
            </div>
          ))}
          </section>
